...
Original file is located at
    https://colab.research.google.com/drive/1EY3CzpOEwh9UCFb-MMfbYJnn_SAEfFse
"""
...
def cold_start_hybrid_with_mood(fav_genres=None, fav_movie=None, fav_actor=None, fav_director=None, mood=None, topn=10):
    ...
    for uid in ratings['userId'].unique():
        liked_movies = ratings[(ratings['userId'] == uid) & (ratings['rating'] >= 4.0)]
        liked_movie_ids = liked_movies['movieId'].values
        liked_indices = movies[movies['movieId'].isin(liked_movie_ids)].index
        if len(liked_indices) == 0:
            continue
        liked_vectors = soup_matrix[liked_indices]
    ...
    similar_user_ids = [u[0] for u in sorted(similarities.items(), key=lambda x: x[1], reverse=True)[:5]]

    movie_scores = {}
    for uid in similar_user_ids:
        user_seen = set(ratings[ratings['userId'] == uid]['movieId'])
        for movie_id in movies['movieId']:
            if movie_id in user_seen:
                continue
            pred = model.predict(uid, movie_id).est
            movie_scores.setdefault(movie_id, []).append(pred)
    movie_scores = {mid: np.mean(score) for mid, score in movie_scores.items()}
